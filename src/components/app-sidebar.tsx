...
type NavItem = { href: string; label: string; icon: typeof LayoutDashboard; roles: Role[]; badge?: number };

export function AppSidebar({
  role, name, unreadMessages, unreadNotifications,
}: { role: Role; name: string; unreadMessages: number; unreadNotifications: number }) {
  ...
  return (
    <Sidebar>
      <SidebarHeader className="gap-3 border-b border-sidebar-border p-4">
        <BrandLogo variant="plate" className="h-12 w-full px-3 py-2" />
              ...
              {visible.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.href}>
                    <SidebarMenuButton
                      size="lg" isActive={active} onClick={closeDrawer}
                    ...
                    >
                      ...
                      {item.badge && item.badge > 0 ? (
                        <Badge className="ml-auto bg-accent text-accent-foreground">{item.badge}</Badge>
                      ) : null}
                    </SidebarMenuButton>
                  </SidebarMenuItem>
                );
      ...
      <SidebarFooter className="border-t border-sidebar-border p-2">
        <SidebarMenu className="gap-1.5">
          <SidebarMenuItem>
            <SidebarMenuButton
              size="lg" isActive={pathname.startsWith("/perfil")} onClick={closeDrawer}
              className="gap-3 rounded-xl px-3 [&_svg]:size-5"
              render={<Link to={"/perfil" as any} />}
            >
