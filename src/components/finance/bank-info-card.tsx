...
export function BankInfoCard({ bank }: { bank: any }) {
  if (!bank || (!bank.bank_name && !bank.pix_key)) {
    return <Card><CardContent className="p-6 text-sm text-muted-foreground">Os dados bancários ainda não foram cadastrados.</CardContent></Card>;
  }
  return (
    <div className="flex flex-col gap-4">
      {bank.bank_name && (
        <Card className="overflow-hidden">
          <CardHeader className="flex flex-row items-center gap-2 border-b border-border bg-muted/40">
            <span className="flex size-8 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Landmark className="size-4" /></span>
      ...
      {bank.pix_key && (
        <Card className="overflow-hidden">
          <CardHeader className="flex flex-row items-center gap-2 border-b border-border bg-gold/15">
            <span className="flex size-8 items-center justify-center rounded-lg bg-gold text-primary"><QrCode className="size-4" /></span>
      ...
      {bank.notes && (
        <div className="flex items-start gap-2 rounded-xl border border-border bg-muted/40 p-3 text-xs text-muted-foreground">
          <Info className="mt-0.5 size-4 shrink-0" /><p>{bank.notes}</p>
        </div>
      )}
    </div>
  );
